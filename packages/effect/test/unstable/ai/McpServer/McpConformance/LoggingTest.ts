  ...
  it.layer(layer)(`Mcp Conformance (${protocol.protocolVersion})`, (it) => {
    describe("Logging", () => {
      ...
      describe("Log Message Notifications", () => {
        ...
        // FIX: The wire notification is implemented, but initialize does not
        // advertise capabilities.logging, so clients cannot discover this
        // supported protocol operation.
        it.effect.skip("MUST emit log messages as notifications without an identifier", () =>
          Effect.gen(function*() {
            const fixture = yield* makeMcpStdioHarness(protocol)
            yield* fixture.initialize()
            yield* fixture.server.notifications["notifications/message"]({
        ...
        // FIX: Effect log events are not connected to MCP's Stdio service or to
        // notifications/message. Until that bridge exists, only explicitly
        // emitted protocol notifications can be checked for stream integrity.
        it.effect.skip("SCENARIO does not corrupt the stdio protocol stream with log output", () =>
          Effect.gen(function*() {
            const fixture = yield* makeMcpStdioHarness(protocol)
            yield* fixture.initialize()
