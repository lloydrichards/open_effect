...
const runElicitation = <S extends Schema.ConstraintEncoder<Record<string, unknown>, unknown>>(
  client: McpSchema.McpReverseClient,
  schema: S
) =>
  McpServer.elicit({
      ...
      describe("Form Mode", () => {
        it.effect("MUST send the message and requested primitive form schema", () =>
          Effect.gen(function*() {
            ...
            const result = yield* peer.client.elicit(request)
        ...
        // FIX: McpServer.elicit generates a JSON Schema containing
        // `additionalProperties: false`, which the June adapter currently rejects
        ...
        // FIX: This is blocked by the same requested-schema projection gap above;
