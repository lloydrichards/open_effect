      ...
      describe("Creating Messages", () => {
        ...
        it.effect.skip("MUST preserve message order and sampling request options", () =>
          Effect.gen(function*() {
            ...
            yield* peer.client.createMessage(samplingRequestWithOptions)
        ...
        it.effect("MUST surface sampling errors returned by the client", () =>
          Effect.gen(function*() {
            ...
            assert.instanceOf(error, McpSchema.McpReverseOperationError)
            assert.strictEqual(error.operation, "sampling/createMessage")
          }).pipe(Effect.scoped))
      })
    })
  })
